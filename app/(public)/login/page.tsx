  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!validateRes.ok) {
        ...
        setGeneralError(data.error || t("login_invalid"));
        setLoading(false);
        return;
      }
    } catch {
      setGeneralError(t("login_error"));
      setLoading(false);
      return;
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#141416] border border-[#2A2A2E] rounded-2xl p-8">
          ...
          <p className="mt-4 text-center text-sm">
            <Link href="/reset-password" className="text-zinc-500 hover:text-[#F9CA1F] transition-colors">
              {t("login_forgot")}
            </Link>
          </p>
          ...
          <p className="mt-3 text-center text-sm text-zinc-500">
            {t("login_no_account")}{" "}
            <Link href="/register" className="text-[#F9CA1F] font-medium hover:underline">
              {t("login_create")}
            </Link>
          </p>
        </div>
